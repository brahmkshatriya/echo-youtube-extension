...
open class EchoLoadSongEndPoint(override val api: YoutubeiApi): LoadSongEndpoint() {
    ...
    private suspend fun parseSongResponse(songId: String, response: HttpResponse, api: YoutubeiApi) = runCatching {
        ...
        val lyricsBrowseId: String? = tabs.getOrNull(1)?.tabRenderer?.endpoint?.browseEndpoint?.browseId
        val relatedBrowseId: String? = tabs.getOrNull(2)?.tabRenderer?.endpoint?.browseEndpoint?.browseId
        ...
        val isExplicit: Boolean = video.badges?.any { it.isExplicit() } == true
